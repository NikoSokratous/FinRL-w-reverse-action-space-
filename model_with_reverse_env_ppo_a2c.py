...
class DRLEnsembleAgent:
    ...
    def DRL_prediction(
        self, model, n_action_space, name, last_state, iter_num, turbulence_threshold, initial
    ):
    ...
    def run_ensemble_strategy(
        ...
        for i in range(
            ...
            self.train_env_reverse = DummyVecEnv(
                [
                    lambda: StockTradingEnv(
                        df=train,
                        stock_dim=self.stock_dim,
            ...
            # print("training: ",len(data_split(df, start=20090000, end=test.datadate.unique()[i-rebalance_window]) ))
            # print("==============Model Training===========")
            print("======A2C REVERSE Training========")
            model_a2c_r = self.get_model(
                "a2c", self.train_env_reverse, policy="MlpPolicy", model_kwargs=A2C_model_kwargs
            )
            model_a2c_r = self.train_model(
                model_a2c_r,
                "a2c",
                tb_log_name=f"a2c_{i}",
                iter_num=i,
            ...
            print(
                "======A2C REVERSE Validation from: ",
                validation_start_date,
                "to ",
                validation_end_date,
            )
            val_env_a2c_reverse = DummyVecEnv(
                [
                    lambda: StockTradingEnv(
                        df=validation,
                        stock_dim=self.stock_dim,
            ...
            val_obs_a2c_r = val_env_a2c_reverse.reset()
            self.DRL_validation(
                model=model_a2c_r,
                test_data=validation,
                test_env=val_env_a2c_reverse,
                test_obs=val_obs_a2c_r,
            )
            sharpe_a2c_r = self.get_validation_sharpe(i, model_name="A2C_r")
            print("A2C REVERSE Sharpe Ratio: ", sharpe_a2c_r)
...
#########################################################################################
            ...
            print("======PPO REVERSE Training========")
            model_ppo_r = self.get_model(
                "ppo", self.train_env_reverse, policy="MlpPolicy", model_kwargs=PPO_model_kwargs
            )
            model_ppo_r = self.train_model(
                model_ppo_r,
                "ppo",
                tb_log_name=f"ppo_{i}",
                iter_num=i,
            ...
            print(
                "======PPO REVERSE Validation from: ",
                validation_start_date,
                "to ",
                validation_end_date,
            )
            val_env_ppo_reverse = DummyVecEnv(
                [
                    lambda: StockTradingEnv(
                        df=validation,
                        stock_dim=self.stock_dim,
            ...
            val_obs_ppo_R = val_env_ppo_reverse.reset()
            self.DRL_validation(
                model=model_ppo_r,
                test_data=validation,
                test_env=val_env_ppo_reverse,
                test_obs=val_obs_ppo_R,
            )
            sharpe_ppo_r = self.get_validation_sharpe(i, model_name="PPO_r")
            print("PPO Reverse Sharpe Ratio: ", sharpe_ppo_r)
            ...
            sharpe_list = [sharpe_ppo,sharpe_a2c,sharpe_a2c_r,sharpe_ppo_r]
            max_sharpe = max(sharpe_list)
            max_index = sharpe_list.index(max_sharpe)

            if sharpe_ppo>=sharpe_ppo_r and sharpe_ppo>=sharpe_a2c and sharpe_ppo>=sharpe_a2c_r:
                model_ensemble = model_ppo
                n_action_space = 1
                model_use.append('PPO')
            elif sharpe_a2c>=sharpe_ppo_r and sharpe_a2c>=sharpe_a2c_r:
                model_ensemble = model_a2c
                n_action_space = 1
                model_use.append('A2C') 
            elif sharpe_ppo_r>=sharpe_a2c_r:
